use crate::{
    ...
    utils::{
        center_to_signed, decompose_balanced, div_round_signed, encode_base_p, mod_q_i64, poly_add,
        poly_mul,
    },
};
...
impl GLWECrypto {
    ...
    pub fn add_ciphertexts(&self, c1: &GLWECiphertext, c2: &GLWECiphertext) -> GLWECiphertext {
        let n = self.params.n;
        let q = self.params.q;
    ...
    pub fn multiply_decomposed(
        ...
        for i in 0..max_digits {
            ...
            let part = GLWECiphertext {
                b: part_b,
                d: part_d,
            };
            acc = self.add_ciphertexts(&acc, &part);
    ...
    pub fn lift_plain_poly_to_q(&self, m_poly_p: &Vec<i64>) -> Vec<i64> {
        ...
        if v.len() < self.params.n {
            v.resize(self.params.n, 0);
        } else if v.len() > self.params.n {
            v.truncate(self.params.n);
        }
        for c in v.iter_mut() {
            *c = ((*c % q) + q) % q;
        }
